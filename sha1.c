 ...
 * (2) VSSサーバ拠点を変更
 * 
 * 5     09/03/27 20:15 Tsujimura543
 * hmac_sha1() を追加
 * 
 * 4     08/11/17 22:15 Tsujimura543
 * static → Thread static に変更
 * 
 * 3     04/11/01 21:02 Tsujimura543
 * 警告レベル 4 での warning を可能な限り減らしてみた
 * 
 * 2     04/07/07 11:54 Tsujimura543
 */
...
unsigned char   *
_sha1( const unsigned char *src, size_t src_len )
{
    SHA1Context             sha;
    unsigned char           *s = NULL;
    int                     err;
    Thread static uint8_t   Message_Digest[1024];

    memset( Message_Digest, 0x00, 1024 );
    ...
    err = SHA1Input( &sha, src, src_len );
    if ( err ) {
        fprintf( stderr, "SHA1Input Error %d. Should be %d.\n",
                 err, shaStateError );
        SHA1Reset( 0 );
        return ( NULL );
    }
...
unsigned char   *
sha1( const unsigned char *src )
{
    return ( _sha1( src, strlen( (char *)src ) ) );
}
