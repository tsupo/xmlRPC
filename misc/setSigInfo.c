 ...
 *
 * $Log: /comm/xmlRPC/misc/setSigInfo.c $
 * 
 * 1     09/05/29 6:59 tsupo
 * VSSサーバ拠点を変更
 * 
 * 3     09/05/27 22:03 Tsujimura543
 * setSigInfo3() を追加
 * 
 * 2     09/05/27 19:28 Tsujimura543
 * setSigInfo() を取り込む
 * 
 * 1     09/05/27 18:57 Tsujimura543
 * bookey/setSigInfo.c を xmlRPC/misc/setSigInfo.c として xmlRPC.dll 中に
 * 取り込む
 * 
 * 2     09/05/27 17:08 Tsujimura543
 * setSigInfo() の引数を追加
 * 
...
#ifndef	lint
static char	*rcs_id =
"$Header: /comm/xmlRPC/misc/setSigInfo.c 1     09/05/29 6:59 tsupo $";
#endif
    ...
    )
{
    ...
    dbaz_data( buf, (const unsigned char *)sig2, strlen(sig2),
               inbuf, outbuf, &crc, &ninb );
    dbaz_data( buf, (const unsigned char *)sig3, strlen(sig3),
               inbuf, outbuf, &crc, &ninb );
